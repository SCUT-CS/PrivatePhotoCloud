...
- 本地缩略图的计算需要两个云端缩略图和一个溢出数组，对于溢出数组的存储提出以下思路：
  ...
  **溢出数组可能泄露信息！**
  ...
  2. ~~利用LSB编码，使用RGB通道的最低位存储数组。~~不可行，LSB不抗压缩，除非云端使用取样的方法计算缩略图。
     - 缺点：RGB通道的最低位需要不加密存储在云端的另一个图像中。另外，此方法取决于云服务计算缩略图的算法，如果是平均值的话会损坏数组，取样的话会降低缩略图结果的画质。
  3. 尝试一种除法同态or全同态算法改进LSB？
  4. 将两张图片延长部分存放数组。
  5. 单独的文件保存在本地或云端。
