...
- **保存时间测试（图片原文）**
  ...
  **密文图片几乎不可压缩，中等大小的HEIF格式的密文图片40mb。采用0%压缩率的webp格式。**
  ...
  - **jpeg格式**（Problem：有损，原Demo的黑色溢出是由jpg的有损压缩带来的）(3次取平均)

    M:

    | 压缩率/% | 时间/ms | 文件大小/mb                  |
    | -------- | ------- | ---------------------------- |
    | 100      | 451     | 10.35（仍然大于原JPG文件？） |
...
- **打开webp时间测试**
  ...
  S：297ms 374ms 404ms 377ms 381ms
...
- **打开HEIF时间测试**
  ...
  S：241ms 192ms 196ms 247ms 196ms
...
- **打开JPG时间测试**
  ...
  S：276ms 269ms 234ms 261ms 269ms
